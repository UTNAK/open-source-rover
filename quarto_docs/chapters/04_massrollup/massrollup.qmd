...
library(igraph)
root <- "OSR"
g <- graph_from_data_frame(df[,c("c2_localname","c1_localname")], directed = TRUE, vertices = NULL)
...
df_mass <- df_mass %>% arrange(factor(c1_localname, levels = names(order)))
df_mass
```
...
```{r}
write_json(df_mass, path = "data.json")
```
