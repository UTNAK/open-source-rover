...
当時のペインポイント
- ツールの問題
- Elyseに聞いてみたいが、結局元のスプレッドシートのやり方に戻ってしまったのはなぜか、ツールの問題か。
...
searchDirectory(4, "osr_common.R", dirname(getwd()))

source(searchDirectory(4, "osr_common.R", dirname(getwd())))
source(searchDirectory(4, "massRollup.R", dirname(getwd())))

a ="aaa"
if( length(a) ){print("yes")}

path <- searchDirectory(4, "build.gradle", dirname(getwd()))
str <- stringr::str_split(path, "build.gradle", simplify = TRUE)[1]
...
a<-list.files(parent_directory, recursive = TRUE, pattern = "pattern", full.names = TRUE)
```
